...
git_init_if_necessary() {
  BREW_OFFICIAL_REMOTE="https://github.com/autobrew/brew"
  CORE_OFFICIAL_REMOTE="https://github.com/autobrew/homebrew-core"
  ...
  [[ -d "$HOMEBREW_LIBRARY/Taps/autobrew/homebrew-core" ]] || return
  safe_cd "$HOMEBREW_LIBRARY/Taps/autobrew/homebrew-core"
  if [[ ! -d ".git" ]]
  then
    set -e
    trap '{ rm -rf .git; exit 1; }' EXIT
...
EOS
  ...
  then
    ...
    [[ -d "$HOMEBREW_LIBRARY/Taps/autobrew/homebrew-core" ]] && brew install git
  ...
  do
    ...
    then
      ...
      [[ -n "$SKIP_FETCH_CORE_REPOSITORY" && "$DIR" = "$HOMEBREW_LIBRARY/Taps/autobrew/homebrew-core" ]] && continue
    fi
